const numberToWords = (num, currencyName = 'SOLES') => {
  var data = {
    numero: num,
    ...
    letrasMonedaPlural: currencyName.toUpperCase(),
  ...
  if (data.centavos > 0) {
    data.letrasCentavos = 'CON ' + (function () {
      ...
      return completeZeros(data.centavos, 2) + '/100';
    })();
  } else {
...
function completeZeros(val, digits) {
  val = String(val);
  if (val.length >= digits) return val;
  return completeZeros('0' + val, digits);
}
